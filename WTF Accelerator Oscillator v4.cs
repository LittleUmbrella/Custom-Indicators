// Accelerator Oscillator Indicator (WTF v4)
...
namespace Forex_Strategy_Builder
{
    ...
    /// </summary>
	public class Accelerator_Oscillator_WTF : Indicator
	{
        /// <summary>
        ...
        /// </summary>
        public Accelerator_Oscillator_WTF(SlotTypes slotType)
        {
            // General properties
            IndicatorName  = "Accelerator Oscillator (WTF v4)";
            ...
            IndParam.ListParam[4].ToolTip  = "Chose higher time frame as compared to current chart period for this indicator.";		
...
/// end WTF modfication 1 version 4
        ...
        public override void Calculate(SlotTypes slotType)
        {
			...
			double[] adMASlow  = MovingAverage(iSlow, 0, maMethod, hfPrice);
			double[] adMAFast  = MovingAverage(iFast, 0, maMethod, hfPrice);
			double[] adAO      = new double[Bars];
			double[] adAC      = new double[Bars];

            for (int iBar = iSlow - 1; iBar < hBars; iBar++)
            {
...
/// start WTF modfication 2 version 4
			...
			double[] hadAC = new double[Bars];
			adAC.CopyTo (hadAC, 0);
			
			int err3 = CurrentTimeFrame(hIndex, hBars, ref adAC);		
...
/// start WTF modfication 3 version 4		
			...
			for (int ctr = 0; ctr < hadAC.Length; ctr++)
			{
				hadAC[ctr] = (hadAC[ctr] < .000000001 && hadAC[ctr] > -.000000001) ? 0 : hadAC[ctr];
			}
			
            OscillatorLogic(iFirstBar, iPrvs, hadAC, dLevel, -dLevel, ref Component[1], ref Component[2], indLogic);
