// Moving Average Indicator -- Wider Time Frame v4
...
{
    /// <summary>
    /// Moving Average Slope Indicator with Draw Only option so can see on chart, but no entry or exit signals
	/// WTF -- wider time frame series
		/// v2 - latest version
		/// v 3 -- no changes
    ...
    /// </summary>
    public class Moving_Average_Slope_WTF : Indicator
    {
        /// <summary>
        /// Sets the default indicator parameters for the designated slot type
        /// </summary>
        public Moving_Average_Slope_WTF (SlotTypes slotType)
        {
            // General properties
            IndicatorName = "Moving Average Slope (WTF v4)";
			...
			IndParam.ListParam[0].ItemList = new string[]
			{
				"The MA Slope rises",
				"The MA Slope falls",
				"The MA Slope is higher than the level",
				"The MA Slope is lower than the level",
				"The MA Slope crosses the level line upward",
				"The MA Slope crosses the level line downward",
				"The MA Slope changes its direction upward",
				"The MA Slope changes its direction downward"
			};
            ...
            IndParam.ListParam[1].Caption  = "MA Smoothing";
            ...
            IndParam.ListParam[3].Caption  = "MA Slope Smoothing";
            ...
            IndParam.ListParam[3].ToolTip  = "The smoothing method of Moving Average Slope.";
            ...
            IndParam.NumParam[1].Caption   = "MA Slope Period";
            IndParam.NumParam[1].Value     = 14;
            IndParam.NumParam[1].Min       = 1;
            IndParam.NumParam[1].Max       = 200;
            IndParam.NumParam[1].Enabled   = true;
            IndParam.NumParam[1].ToolTip   = "The Moving Average Slope period.";
            ...
            IndParam.NumParam[2].ToolTip   = "Level for Indicator Logic.";
...
/// end WTF modfication 1 version 4
            ...
            IndParam.CheckParam[1].Caption = "Use Absolute Values";
            IndParam.CheckParam[1].Checked = false;
            IndParam.CheckParam[1].Enabled = true;
            IndParam.CheckParam[1].ToolTip = "Use Aboslute Slope values instead of positive or negatives.";
        ...
        public override void Calculate(SlotTypes slotType)
        {
			...
			MAMethod  maSlopeMethod = (MAMethod )IndParam.ListParam[3].Index;
            int       iMAPeriod  = (int)IndParam.NumParam[0].Value;
            int       iMASlopePeriod   = (int)IndParam.NumParam[1].Value;
			double dLevel = IndParam.NumParam[2].Value;
            int       iPrvs    = IndParam.CheckParam[0].Checked ? 1 : 0;
			bool bAbsolutes = IndParam.CheckParam[1].Checked;
			...
			int err2 = HigherBasePrice(price, hBars, hfHigh, hfLow, hfOpen, hfClose, out hfPrice);	
			if (err1 ==1) return;


            // Calculation
            int iFirstBar = Math.Max(iMAPeriod, iMASlopePeriod) + 2 + iPrvs;
			double[] adMASlope = new double[Bars];
			double[] adMASlopeSmoothed = new double[Bars];

            double[] adMA = MovingAverage(iMAPeriod, 0, maMethod, hfPrice);

			for (int iBar = iFirstBar; iBar < Bars; iBar++ )
			{
				if (bAbsolutes)
				{
					adMASlope[iBar] = Math.Abs(adMA[iBar] - adMA[iBar-1]) / Point;
				}
				else 
				{
					adMASlope[iBar] = (adMA[iBar] - adMA[iBar-1]) / Point;
				}
			}

			adMASlopeSmoothed = MovingAverage(iMASlopePeriod, 0, maSlopeMethod, adMASlope);


			// Convert to Current Time Frame ----------------------------------------------
/// start WTF modfication 2 version 4
			// copy of wider time frame array of values
			double[] hadMASlopeSmoothed = new double[Bars];
			adMASlopeSmoothed.CopyTo (hadMASlopeSmoothed, 0);
			
			int err3 = CurrentTimeFrame(hIndex, hBars, ref adMASlopeSmoothed);		
...
/// end WTF modfication 2 version 4
			...
			Component[0].CompName   = "MA Slope";
			Component[0].DataType   = IndComponentType.IndicatorValue;
			Component[0].ChartType  = IndChartType.Histogram;
			Component[0].FirstBar   = iFirstBar;
			Component[0].Value      = adMASlopeSmoothed;
			...
			switch (IndParam.ListParam[0].Text)
			{
				...
				case "The MA Slope rises":
					indLogic = IndicatorLogic.The_indicator_rises;
					dLevel = 0;
					break;

				case "The MA Slope falls":
					indLogic = IndicatorLogic.The_indicator_falls;
					dLevel = 0;
					break;

				case "The MA Slope is higher than the level":
					indLogic = IndicatorLogic.The_indicator_is_higher_than_the_level_line;
					break;
				
				case "The MA Slope is lower than the level":
					indLogic = IndicatorLogic.The_indicator_is_lower_than_the_level_line;
					break;

                case "The MA Slope crosses the level line upward":
                    indLogic = IndicatorLogic.The_indicator_crosses_the_level_line_upward;
                    break;

                case "The MA Slope crosses the level line downward":
                    indLogic = IndicatorLogic.The_indicator_crosses_the_level_line_downward;
                    break;

                case "The MA Slope changes its direction upward":
                    indLogic = IndicatorLogic.The_indicator_changes_its_direction_upward;
                    break;

                case "The MA Slope changes its direction downward":
...
/// start WTF modfication 3 version 4		
			...
			for (int ctr = 0; ctr < hadMASlopeSmoothed.Length; ctr++)
			{
				hadMASlopeSmoothed[ctr] = (hadMASlopeSmoothed[ctr] < .000000001 && hadMASlopeSmoothed[ctr] > -.000000001) ? 0 : hadMASlopeSmoothed[ctr];
			}

			if (bAbsolutes)
			{
				NoDirectionOscillatorLogic(iFirstBar, iPrvs, hadMASlopeSmoothed, dLevel, ref Component[1], indLogic);
				Component[2].Value = Component[1].Value;
				SpecialValues = new double[1] { dLevel };
			}
			else {
				OscillatorLogic(iFirstBar, iPrvs, hadMASlopeSmoothed, dLevel, -dLevel, ref Component[1], ref Component[2], indLogic);
        ...
        public override string ToString()
        {
            string sString = IndicatorName + 
                (IndParam.CheckParam[0].Checked ? "* (" : " (") +
					...
					"MA:" + IndParam.NumParam[0].ValueToString + ", " + 
					"Slope:" + IndParam.NumParam[1].ValueToString + ", " +
					"Level:" + IndParam.NumParam[2].ValueToString + ", " +
